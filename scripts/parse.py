...
class Reflector:
    ...
    TYPE_ALIASES = {"char*": "str", "constchar*": "conststr", "unsigned int": "uint"}
...
"""
    ...
    def generate_type_setter(self, type_name: str, type_enum: str) -> str:
        normalized_type = type_name.replace(" ", "") if "*" in type_name else type_name

        if normalized_type in Reflector.TYPE_ALIASES:
            type_suffix = Reflector.TYPE_ALIASES[normalized_type]
        else:
            type_suffix = type_name.replace("*", "ptr").replace(" ", "_")

        return f"DEFINE_FIELD_SETTER({type_suffix}, {type_enum}, {type_name})"
